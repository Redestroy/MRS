...
ATask::ATask(): Task(),commands()
{
	...
	SetType(TypeDefinitions::TASK_ATOMIC);
}
...
{
	size_t sizee = taskString->length();
	//printf(" this is task");
	this->SetAttributesFromString(taskString, sizee);
}
...
void ATask::SetAttributesFromString(std::string * taskString, size_t size)
{
	...
	try {
		...
		SetState(TypeDefinitions::GetTaskState(std::stoi(taskString->substr(currentSpace + 1, nextSpace - currentSpace))));// check value
	}
	catch (...) {
	...
	switch (TypeDefinitions::CharToCondition(startType)) {
	...
	switch (TypeDefinitions::CharToCondition(endType)) {
		case TypeDefinitions::CONDITION_POSITION:
	...
	for (int i = 0; i < commandNum; i++) {
		...
		this->Add(Action(&taskSubstring.substr(firstTemp + 1, secondTemp - firstTemp - 1)));//

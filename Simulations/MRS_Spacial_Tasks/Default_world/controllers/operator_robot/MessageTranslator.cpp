...
int MessageTranslator::getTaskIndex(std::string * packet)
{
	...
	case TypeDefinitions::TASK_COMPLETE_MESSAGE:
		std::cout << "Task index " << packet->substr(packet->find_first_of(' ') + 1, packet->find_first_of(' ', packet->find_first_of(' ') + 1) - (packet->find_first_of(' '))).c_str();
		return std::stoi(packet->substr(packet->find_first_of(' ')).c_str());
		//return std::stoi(packet->substr(packet->find_first_of(' ') + 1, packet->find_first_of(' ', packet->find_first_of(' ') + 1) - (packet->find_first_of(' '))).c_str());
